...
import { authLimiter, refreshLimiter, logoutLimiter, meLimiter } from '../middleware/rate-limit.middleware.js';
...
router.post('/logout', logoutLimiter, asyncHandler(authController.logout));
router.post('/refresh', refreshLimiter, asyncHandler(authController.refresh));
router.get('/me', requireAuth, meLimiter, asyncHandler(authController.me));
